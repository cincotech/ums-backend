...
class GuestRequest(models.Model):
    ...
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='guest_request')
    requested_role = models.ForeignKey(Role, on_delete=models.RESTRICT, null=True, blank=True)
    ...
    reviewed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='reviewed_guests')
...
class GuestDocument(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    guest_request = models.ForeignKey(GuestRequest, on_delete=models.CASCADE, related_name='documents')
    ...
    verified_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='verified_guest_docs')
    rejection_reason = models.TextField(null=True, blank=True)
...
class GuestNotification(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    guest_request = models.ForeignKey(GuestRequest, on_delete=models.CASCADE, related_name='notifications')
    type = models.CharField(max_length=20, choices=TYPE_CHOICES, default='info')
    title = models.CharField(max_length=255)
    message = models.TextField()
    action_url = models.CharField(max_length=500, null=True, blank=True)
    is_read = models.BooleanField(default=False)
    document = models.ForeignKey(GuestDocument, on_delete=models.SET_NULL, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
...
class RoleDocumentRequirement(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    role = models.ForeignKey(Role, on_delete=models.CASCADE, related_name='document_requirements')
    document_type = models.CharField(max_length=30, choices=GuestDocument.DOCUMENT_TYPES)
