...
def guest_profile(request):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            # Get stats
            total_docs = RoleDocumentRequirement.objects.filter(
                role=guest_request.requested_role
            ).count()
            ...
            requirements = RoleDocumentRequirement.objects.filter(
                role=guest_request.requested_role
            )
            requirements_serializer = DocumentRequirementSerializer(
        ...
        return error_response(
            message="Guest request not found", status_code=status.HTTP_404_NOT_FOUND
        )
...
def account_status(request):
    ...
    try:
        ...
        return success_response(
            data={"status": guest_request.status}, message="Status retrieved"
        )
    except GuestRequest.DoesNotExist:
        return error_response(
            message="Guest request not found", status_code=status.HTTP_404_NOT_FOUND
...
def documents(request):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            serializer = GuestDocumentSerializer(
            ...
            if serializer.is_valid():
                ...
                result_serializer = GuestDocumentSerializer(
...
def role_document_requirements(request, role_id):
    ...
    try:
        ...
        return success_response(
            data=serializer.data, message="Requirements retrieved successfully"
        )
    except Exception as e:
        return error_response(
            message=f"Error: {str(e)}",
