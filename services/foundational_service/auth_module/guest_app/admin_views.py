...
class AccountRequestViewSet(BaseViewSet):
    ...
    queryset = GuestRequest.objects.select_related('user', 'requested_role', 'reviewed_by').prefetch_related('documents')
    permission_classes = [IsAuthenticated]
    search_fields = ['user__email', 'user__first_name', 'user__last_name', 'requested_role__name']
    ...
    def stats(self, request):
        ...
        stats_data = {
            ...
            'total_account_types': GuestRequest.objects.values('requested_role').distinct().count(),
            'pending_review': GuestRequest.objects.filter(status='pending').count(),
            'total_documents': GuestDocument.objects.count(),
            'rejected_documents': GuestDocument.objects.filter(status='rejected').count(),
            'rejected_accounts': GuestRequest.objects.filter(status='rejected').count(),
        }
        serializer = AccountRequestStatsSerializer(stats_data)
        return success_response(data=serializer.data, message='Stats retrieved successfully')
    ...
    def review(self, request, pk=None):
        ...
        result_serializer = AccountRequestSerializer(guest_request, context={'request': request})
        return success_response(data=result_serializer.data, message=f'Request {review_status}')
    ...
    def documents(self, request, pk=None):
        ...
        serializer = AccountRequestDocumentSerializer(documents, many=True, context={'request': request})
    ...
    def update_document(self, request, pk=None, document_id=None):
        ...
        try:
            ...
            result_serializer = AccountRequestDocumentSerializer(document, context={'request': request})
            return success_response(data=result_serializer.data, message='Document updated')
        except GuestDocument.DoesNotExist:
