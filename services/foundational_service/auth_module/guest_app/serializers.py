...
class GuestDocumentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = GuestDocument
        fields = ['id', 'name', 'type', 'status', 'uploaded_at', 'url', 'file_size', 
                  'mime_type', 'rejection_reason', 'verified_at', 'verified_by']
...
class GuestNotificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'type', 'title', 'message', 'action_url', 'is_read', 'created_at', 'details']
...
class DocumentRequirementSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['type', 'label', 'description', 'required', 'max_size', 'accepted_formats']
...
class GuestProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = GuestRequest
        fields = ['id', 'email', 'first_name', 'last_name', 'phone', 'birth_date', 
                  'address', 'profile_image_url', 'status', 'requested_role', 
                  'profile_submitted', 'profile_submitted_at', 'reviewed_at', 
                  'rejection_reason', 'created_at', 'updated_at']
        read_only_fields = ['id', 'status', 'profile_submitted', 'profile_submitted_at', 
...
class GuestUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = GuestRequest
        fields = ['id', 'email', 'first_name', 'last_name', 'phone', 'birth_date', 
                  'address', 'profile_image_url', 'created_at', 'updated_at', 'status', 
                  'requested_role', 'documents', 'notifications', 'progress_steps', 
                  'profile_submitted', 'profile_submitted_at', 'reviewed_at', 
    ...
    def get_progress_steps(self, obj):
        steps = [
            {'id': 1, 'title': 'Create Account', 'description': 'Register and verify email', 'completed': True, 'current': False},
            {'id': 2, 'title': 'Complete Profile', 'description': 'Fill in personal information', 'completed': obj.profile_submitted, 'current': not obj.profile_submitted},
            {'id': 3, 'title': 'Upload Documents', 'description': 'Submit required documents', 'completed': obj.documents.filter(status='verified').count() > 0, 'current': obj.profile_submitted and obj.status == 'pending'},
            {'id': 4, 'title': 'Under Review', 'description': 'Application being reviewed', 'completed': obj.status in ['approved', 'rejected'], 'current': obj.status == 'under_review'},
            {'id': 5, 'title': 'Decision', 'description': 'Application approved or rejected', 'completed': obj.status in ['approved', 'rejected'], 'current': False},
        ]
        return steps
