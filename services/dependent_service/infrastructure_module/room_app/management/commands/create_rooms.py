...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if building_id:
            ...
            if not building:
                self.stdout.write(
                    self.style.ERROR("No building found. Create a building first.")
                )
                return
        ...
        for start, end in ranges:
            for num in range(start, end + 1):
                ...
                if not Room.objects.filter(
                    room_code=room_code, building=building
                ).exists():
                    Room.objects.create(
                        building=building,
                        room_name=room_name,
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Successfully created {created_count} rooms in {building.building_name}"
            )
        )
