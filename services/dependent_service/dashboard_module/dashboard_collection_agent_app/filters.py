...
from .models import Bank, Wording, FeesSheet, PaymentInstallement, PaymentReminder, PaymentPlan, PaymentPromise, Payment, CollectionCorrespondence
...
class WordingFilter(django_filters.FilterSet):
    ...
    def filter_search(self, queryset, name, value):
        return queryset.filter(
            Q(wording_name__icontains=value)
        )
...
class FeesSheetFilter(django_filters.FilterSet):
    ...
    def filter_search(self, queryset, name, value):
        return queryset.filter(
            Q(wording__wording_name__icontains=value) |
            Q(base_amount__icontains=value)
        )
...
class PaymentReminderFilter(django_filters.FilterSet):
    ...
    def filter_search(self, queryset, name, value):
        return queryset.filter(
            Q(student__matricule__icontains=value) |
            Q(reminder_type__icontains=value)
        )
