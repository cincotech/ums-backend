...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--students-per-class', type=int, default=10, help='Number of students per class')
            ...
            for class_obj in classes:
                ...
                for i in range(students_per_class):
                    ...
                    Inscription.objects.create(
                        ...
                        is_year_close=False,
                        date_inscription=fake.date_between(start_date='-1y', end_date='today'),
                    )

                    total_created += 1

                self.stdout.write(
                    self.style.SUCCESS(f'Created {students_per_class} students for {class_obj.class_name}')
                )

        self.stdout.write(
            self.style.SUCCESS(f'Successfully created {total_created} students across {classes.count()} classes')
        )
