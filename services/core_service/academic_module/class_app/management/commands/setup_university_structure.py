    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            for faculty in faculties:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Created dean for {faculty.faculty_name}")
                )
            ...
            for class_obj in classes:
                ...
                class_groups = ClassGroup.objects.filter(
                    class_fk=class_obj, academic_year=academic_year
                )
                    ...
                    for group in class_groups:
                        ...
                        for _ in range(1):  # 3 timetables per group
                            ...
                            for attempt in range(max_attempts):
                                generated_start = fake.date_between(
                                    start_date="today", end_date="+30d"
                                )
                                generated_end = generated_start + relativedelta(
                                    months=12
                                )
                                ...
                                print(
                                    f"Skipping timetable creation for group {group.id} due to repeated conflicts."
                                )
                                continue
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Created {total_timetables} timetables")
            )
