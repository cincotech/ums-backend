...
def clean_invalid_attributions():
    ...
    problematic = Attribution.objects.filter(
        principal_teacher_id__isnull=False,
        substitute_teacher_id__isnull=False
    ).filter(
        principal_teacher_id=F('substitute_teacher_id')
    )
    ...
    if count > 0:
        ...
        for attr in problematic.select_related('principal_teacher', 'substitute_teacher', 'course'):
            print(f"  - ID: {attr.id}")
